...
from graphics.graphics_functions import set_pyglet_resource_paths
...
class ImageLoader:
	...
	def load_ui_images(self, graphics_data: GraphicsData) -> None:
		# for key in image name list
		for ui_member, key_value in UIImageNames.list():
			file_name: str = graphics_data.image_filenames[key_value]
			self.ui_images[ui_member] = image(file_name)
	
	def load_game_images(self, graphics_data: GraphicsData) -> None:
		# for key in image name list
